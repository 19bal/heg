...
%        A = SOCR_FITELLIPSE(X,Y) returns the parameters of the best-fit
...
if nargin == 0
  ...
  NoiseLevel = 2.5; % Will add Gaussian noise of this std.dev. to points
...
% % + Nurettin Senyer, 20.Haziran.2006
% Eklenti
%   Elips icin tek bir gosterim bicimi kullanalim. Daha uzun yaricapli olan
%   y (v) ekseni olsun.
% % % 
if Ru > Rv,
    thetarad = thetarad + deg2rad(90);
    tmp = Ru;
    Ru = Rv;
    Rv = tmp;
end;
% % % 
% % % % < BASLA
% % % % Rev 1: 24.08.2007, nurettins
% % % % ThetaDeg (-90, +90) araligina getirildi.
% % % if thetarad > (pi/2),
% % %     thetarad = thetarad - pi;
% % % elseif thetarad < (-pi/2),
% % %     thetarad = thetarad + pi;
% % % end
% % % %   BITIR >
a = [uCentre, vCentre, Ru, Rv, thetarad];

% % + Nurettin Senyer, 01.Temmuz.2006
% Eklenti
% Aci tanimi +y eksenine gore, saat yonu pozitif yondu.
% Simdi ise  +x eksenine gore ve saatin tersi yon pozitif yon olacak.
% 1. thetarad = deg2rad(90) - thetarad;
% 2. a = [uCentre, vCentre, Ru, Rv, thetarad];
% % yukaridaki iki satir hatalara neden oldugundan cikartildi.
